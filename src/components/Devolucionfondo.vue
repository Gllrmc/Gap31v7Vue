    ...
    <v-layout align-start>
        <v-flex>
            <v-toolbar flat color="white">
                <v-toolbar-title>Devolucion de Fondos - Pedidos</v-toolbar-title>
                <v-snackbar
                    v-model="snackbar"
                    :timeout="timeout"
                    right
            ...
            <v-dialog v-model="dialog" max-width="1400px">
                <v-card>
                    <v-card-title>
                    <span class="headline">Devolucion Fondos Proyecto #{{orden}} | Pedido #{{numpedido}}</span>
                    </v-card-title>            
                    <v-card-text>
                        ...
                        <v-dialog v-model="adModal" max-width="290">
                            <v-card>
                                <v-card-title class="headline" v-if="adAccion==1">¿Activar Devolución de Fondo?</v-card-title>
                                <v-card-title class="headline" v-if="adAccion==2">¿Desactivar Devolución de Fondo?</v-card-title>
                                <v-card-text>
                                    Estás a punto de 
                                    <span v-if="adAccion==1">Activar </span>
                                    <span v-if="adAccion==2">Desactivar </span>
                                    la devolución de fondo de {{ adNombre }}
                                </v-card-text>
                                <v-card-actions>
                                    <v-spacer></v-spacer>
                        ...
                        <v-dialog v-model="adModal2" max-width="290">
                            <v-card>
                                ...
                                <v-card-text>
                                    ...
                                    la distribución de fondo de {{ adNombre }} como rendido
                                </v-card-text>
                                <v-card-actions>
                                    <v-spacer></v-spacer>
                                    <v-btn color="green darken-1" flat="flat" @click="activarDesactivarCerrar">
                        ...
                        >
                            <template slot="items" slot-scope="props">
                                <td class="justify-center layout px-20">
                                    ...
                                    <template v-if="props.item.devolucion && props.item.activo">
...
<script>
    ...
    export default {
            data: () => {
                ...
                headersDetalle: [
                    { text: 'Opciones', value: 'opciones', sortable: false },
                    { text: 'Responsable de la Distribucion de Fondos', value: 'responsable', sortable: true },
            ...
            select(){
                ...
                if (this.$store.state.usuario.rol =='Administrador' || this.$store.state.usuario.rol =='JefeAdministracion' ){
                    ...
                    axios.get('api/Proyectos/SelectDeUsuario/'+me.$store.state.usuario.idusuario,configuracion).then(function(response){
                        //console.log(response);
                        proyectosArray=response.data;
                        proyectosArray.map(function(x){
                            me.proyectos.push({text: x.orden + ': ' + x.proyecto,value:x.idproyecto});
