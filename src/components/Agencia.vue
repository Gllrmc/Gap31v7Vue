...
<script>
    ...
    export default {
        data(){
            return {          
                ...
                headers: [
                    ...
                    { text: 'Id', value: 'idagencia', sortable: true },
        ...
        methods:{
            crearPDF(){
                var columns = [
                    {title: "Id", dataKey: "idagencia"},
                    {title: "Agencia", dataKey: "agencia"},
                    {title: "Estado", dataKey: "activo"},
                ];
                var rows = [];

                this.agencias.map(function(x){
                    rows.push({idagencia:x.idagencia,agencia:x.agencia,activo:x.activo ? "Activo" : "Inactivo"});
                });
                ...
                doc.autoTable(columns, rows, {
                    ...
                    addPageContent: function(data) {
                        doc.text("Listado de Agencias", 40, 30);
                    }
                });
            ...
            validar(){
                ...
                if (this.agencia.length<3 || this.agencia.length>50){
                    this.validaMensaje.push("El nombre de la agencia no debe tener menos de 3 caracteres y mas de 50 caracteres.");
                }
            ...
            activarDesactivarMostrar(accion,item){
                this.adModal=1;
                this.adNombre=item.agencia;
                this.adId=item.idagencia;                
                if (accion==1){
                    this.adAccion=1;
                }
                else if (accion==2){
            ...
            activar(){
                ...
                let header={"Authorization" : "Bearer " + this.$store.state.token};
                let configuracion= {headers : header};
                axios.put('api/Agencias/Activar/'+this.adId,{},configuracion).then(function(response){
                    me.adModal=0;
                    me.adAccion=0;
                    me.adNombre="";
                    me.adId="";
            ...
            desactivar(){
                ...
                let header={"Authorization" : "Bearer " + this.$store.state.token};
                let configuracion= {headers : header};
                axios.put('api/Agencias/Desactivar/'+this.adId,{},configuracion).then(function(response){
                    me.adModal=0;
                    me.adAccion=0;
                    me.adNombre="";
                    me.adId="";
