    ...
    <v-layout align-start>
        <v-flex>
            ...
            <v-dialog v-model="dialog" max-width="1500px" persistent>
                <v-card>
                    <v-card-title>
                    <span class="headline">Director's fees para Prospecto #{{orden}}: {{proyecto}} </span>
                    </v-card-title>            
                    <v-card-text>
                                            ...
                                            <v-flex xs12>
                                                <form enctype="multipart/form-data">
                                                    <div class="field">
                                                        ...
                                                        <v-btn @click="onSaveAs(pdfdirfee)">Desc Fee</v-btn>
                                                </form> 
                                            </v-flex>
                                            <v-flex xs12>
                                                <div>       
                        ...
                        <v-dialog v-model="adModal" max-width="400">
                            <v-card>
                                <v-card-title class="headline" v-if="adAccion==1">¿Activar el Fee?</v-card-title>
                                <v-card-title class="headline" v-if="adAccion==2">¿Desactivar el Fee?</v-card-title>
                                <v-card-text>
                                    Estás a punto de 
                        ...
                        <v-data-table
                            ...
                            <template slot="items" slot-scope="props">
                                ...
                                <td>{{ props.item.iddirfee }}</td>
                                <td>{{ props.item.numdirfee }}</td>
                                <td>{{ (props.item.fecdirfee) ? props.item.fecdirfee.substr(0,10) : "" }}</td>
                                <td class="text-xs-right">{{ formatPrice(props.item.impdirfee) }}</td>
                                <td>{{ props.item.pdfdirfee }}</td>
...
<script>
    ...
    export default {
            data: () => {
                ...
                headersDetalle: [
                    { text: 'Opciones', value: 'opciones', sortable: false },
                    { text: '#Id', value: 'iddirfee', sortable: true },
                    { text: '#Sec', value: 'numdirfee', sortable: true },
                    { text: 'Fecha Registro', value: 'fecdirfee', sortable: true },
                    { text: 'Importe', value: 'impdirfee', sortable: true },
                    { text: 'PDF', value: 'pdfdirfee', sortable: true },
            ...
            computed: {
                formTitle () {
                    return this.editedIndex === -1 ? 'Nuevo Fee' : 'Actualizar Fee'
                },
                calcularTotal:function(){
                        var subtotal=0.0;
                        for(var i=0;i<this.dirfees.length;i++){
                            subtotal+=(this.dirfees[i].activo?this.dirfees[i].impdirfee:0);
                        }
                        return subtotal;
                    },                       
                },
            ...
            listarDetail(){
                ...
                axios.get('api/Dirfees/ListarLimbo/'+this.idlimbo,configuracion).then(function(response){
                    // console.log(response);
                    me.dirfees=response.data;
                }).catch(function(error){
                    me.snacktext = 'Se detectó un error. Código: '+ error.response.status;
                    me.snackbar = true;
                    console.log(error);
            ...
            editDetailItem (item) {
                this.iddirfee = item.iddirfee;
                this.numdirfee = item.numdirfee;
                this.fecdirfee = (item.fecdirfee) ? item.fecdirfee.substr(0, 10) : "";
                this.impdirfee = item.impdirfee;
                this.pdfdirfee = item.pdfdirfee;
                this.originaldirfeeguid = item.pdfdirfee;
                this.nota = item.nota;
                this.iduseralta = item.iduseralta;
                this.fecalta = item.fecalta;
                this.iduserumod = item.iduserumod;
                this.fecumod = item.fecumod;
                this.onDownload(this.pdfdirfee);
                this.editedIndex=1;
            ...
            deleteItem (item) {
                ...
                if (resulta) {
                    ...
                    let configuracion= {headers : header};
                    axios.delete('api/Dirfees/Eliminar/'+item.iddirfee,configuracion).then(function(response){
                        me.closeForm();
                        me.limpiarDetail();                        
                        me.listarDetail();
                    }).catch(function(error){
            ...
            guardar () {
                ...
                let configuracion= {headers : header};
                if (me.editedIndex > -1) {
                    if (me.pdfdirfee && me.originaldirfeeguid != me.pdfdirfee ){
                        me.onDelete(me.originaldirfeeguid);
                        me.originaldirfeeguid='';
                        me.onUpload();
                    }
                    //Código para editar
                    //Código para guardar
                    // console.log(me);
                    axios.put('api/Dirfees/Actualizar',{
                        'iddirfee': me.iddirfee,
                        'fecdirfee': me.fecdirfee.substr(0,10),
                        'impdirfee': me.impdirfee,
                        'pdfdirfee': me.pdfdirfee,
                        'nota': me.nota,
            ...
            activarDesactivarMostrar(accion,item){
                this.adModal=1;
                this.adNombre='#' + item.numdirfee + ' ' + item.fecdirfee.substr(0,10) + ' $ ' + item.impdirfee;
                this.adId=item.iddirfee;                
                if (accion==1){
                    this.adAccion=1;
                }
                else if (accion==2){
            ...
            activar(){
                ...
                let header={"Authorization" : "Bearer " + this.$store.state.token};
                let configuracion= {headers : header};
                axios.put('api/Dirfees/Activar/'+this.adId,{},configuracion).then(function(response){
                    me.adModal=0;
                    me.adAccion=0;
                    me.adNombre="";
                    me.adId="";
            ...
            desactivar(){
                ...
                let header={"Authorization" : "Bearer " + this.$store.state.token};
                let configuracion= {headers : header};
                axios.put('api/Dirfees/Desactivar/'+this.adId,{},configuracion).then(function(response){
                    me.adModal=0;
                    me.adAccion=0;
                    me.adNombre="";
                    me.adId="";
