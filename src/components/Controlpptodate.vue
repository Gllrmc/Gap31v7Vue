    ...
    <v-layout align-start>
        <v-flex>
            <v-toolbar flat color="white">
                <v-toolbar-title>Control Presupuesto Vs Real</v-toolbar-title>
                <v-snackbar
                    v-model="snackbar"
                    :timeout="timeout"
                    right
            ...
            <v-dialog v-model="dialog" max-width="1500px" persistent>
                ...
                <v-card>
                    <v-card-title>
                    <span class="headline">Control Real vs Presupuesto del Proyecto #{{orden}}: {{proyecto}} </span>
                    </v-card-title>            
                    <v-card-text>                                      
                <export-excel
                    :data   = "json_data"
                    ...
                    <v-btn color="primary" dark class="mb-2" @click="crearXLS()">Exportar XLS</v-btn>
                        ...
                        <v-flex class="text-xs-right">
                            <strong>Total Presupuesto Vs (Real + Comp.): </strong>$ {{totalPagado=(calcularTotal).toFixed(2)}}
                        </v-flex>
                    </v-card-text>
                    <v-card-actions>
                        <v-spacer></v-spacer>
...
<script>
    ...
    export default {
            ...
            computed: {
                calcularTotal:function(){
                    ...
                    for(var i=0;i<this.controlproyectos.length;i++){
                        resultado=resultado+this.controlproyectos[i].importe;
                    }
                    return resultado;
                }                             
            },
            ...
            listarDetail(){
                ...
                let configuracion= {headers : header};
				//console.log(configuracion);
                axios.get('api/Presupuestos/Controlproyectodate/'+this.idproyecto,configuracion).then(function(response){
                    // console.log(response);
