    ...
    <v-layout align-start>
        <v-flex>
            <v-toolbar flat color="white">
                <v-toolbar-title>Control Motion</v-toolbar-title>
                <v-snackbar
                    v-model="snackbar"
                    :timeout="timeout"
                    right
    ...
    export default {
            ...
            computed: {
                calcularTotal:function(){
                    var resultado=0.0;
                    for(var i=0;i<this.controlmotions.length;i++){
                        resultado=resultado+this.controlmotions[i].imptotal;
                    }
                    return resultado;
                }                             
            },
            ...
            crearXLS(){
                ...
                this.json_data = [];
                for (var x=0; x<this.controlmotions.length; x++){
                    for (let i=0; i<this.items.length; i++){
                        if (this.items[i]["value"]===this.controlmotions[x]["iditem"]){
                            this.json_data.push(this.controlmotions[x]);
                            break
                        }
                    }
                }
            ...
            listarDetail(){
                ...
                let configuracion= {headers : header};      
                //console.log(configuracion);
                axios.get('api/Realmotions/Controlmotion2date/'+this.idproyecto,configuracion).then(function(response){
                    //console.log(response);
                    me.controlmotions=response.data;
                }).catch(function(error){
                    me.snacktext = 'Se detectó un error. Código: '+ error.response.status;
                    me.snackbar = true;
                    console.log(error);
